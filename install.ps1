...
function Get-LatestSnapshotVersion {
    ...
    $ApiUrl = "https://api.github.com/repos/allen0039/komari_agent/releases?per_page=100"
    $ApiUrls = @($ApiUrl)
    if ($GitHubProxy -ne "") {
...
if ($PanelEndpoint -ne "") {
    ...
    if ($InstallVersion -ne "") {
        ...
        $ApiUrl = "https://api.github.com/repos/allen0039/komari_agent/releases/latest"
        try {
    ...
    $BinaryName = "komari-agent-windows-$arch.exe"
    $DownloadUrl = if ($GitHubProxy) { "$GitHubProxy/https://github.com/allen0039/komari_agent/releases/download/$versionToInstall/$BinaryName" } else { "https://github.com/allen0039/komari_agent/releases/download/$versionToInstall/$BinaryName" }
}
