...
这是 [komari_air](https://github.com/allen0039/komari_air) 使用的独立 Agent 仓库，基于 `komari-monitor/komari-agent@c7bafb79b1a73ed9e14e2c248ae4d4163e506036` 修改，保留原 MIT 许可证。

Agent 与主控分别发布版本。推送 `vX.Y.Z` tag 会生成 Linux、macOS、Windows 的 amd64/arm64 二进制、安装脚本和校验清单。主控通过自己的 `AGENT_VERSION` 固定内置 Agent 版本；更新 Agent 仓库不会自动改变已部署主控提供的一键升级版本。发布新版 Agent 后，还需更新主控的 `AGENT_VERSION` 并重新构建、部署主控镜像。

直接构建：`go build -o komari-agent .`。正式版本由 GitHub Actions 将去掉 `v` 前缀的版本号写入二进制。
